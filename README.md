...
- API 批次严格串行，避免并发请求风暴；
- 默认使用流式响应，28 秒分别限制“开始响应”和“开始后完成”两个阶段；
- 模型名精确为 `grok-4.5` 时使用 low 推理；其他模型不注入供应商专属参数；
- 429 与 5xx 最多重试一次；
...
```text
dist/dumber-0.3.1.zip
dist/dumber-0.3.1.zip.sha256
dist/dumber-0.3.1-source.zip
dist/dumber-0.3.1-source.zip.sha256
```
