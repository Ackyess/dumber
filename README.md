...
最终推广必须同时满足：模型建议推广、刺激分达到当前门槛、持久价值低于当前上限，且用户没有对同一内容选择“这条真有用”。“挑选严格度”滑杆同时控制两道门：最左为刺激分 `>= 0.50`、持久价值 `<= 0.65`；默认仍为 `>= 0.72 / <= 0.45`；最右为 `>= 0.95 / <= 0.25`。项目、研究、作品和教程在默认档仍受原有保护。
...
```text
dist/dumber-0.3.6.zip
dist/dumber-0.3.6.zip.sha256
dist/dumber-0.3.6-source.zip
dist/dumber-0.3.6-source.zip.sha256
```
