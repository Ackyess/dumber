...
`schemaVersion < 2` 的旧设置不会迁移 API Key。这是针对 v0.1 源码凭据泄露的显式安全迁移；当前设置 schema 为 V4。
...
第二阶段只在本地最终判定为推广后启动。模型最多返回 5 个正文原文爆点词，每个不得超过 12 个字符；共享层再次核对它们确实逐字存在，拒绝长句、改写或幻觉内容。第二阶段不参与首轮判定，也不能延迟或撤销首轮视觉结果。
...
- 二轮爆点词只包裹原有文本节点，使用分词色板、渐变文字、transform、filter 与 opacity 做错峰霓虹、放大、轻摆和亮度脉冲；reduced-motion 下保留静态霓虹。
...
活动指示器直接观察现有 `data-dumber-state`，聚合扫描、排队、请求、完成和错误数量，并用内容指纹维护不受 X 虚拟列表回收影响的会话已分析/已精选计数；使用 Thinking Orbs 的 Composing 点阵几何与 DUMBER 霓虹色板绘制唯一 Canvas2D 球体，不复制请求状态，也不进入页面布局。
