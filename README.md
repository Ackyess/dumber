...
- 首轮命中后立即出现原版青、蓝、紫、粉、黄虹彩边缘和冷色环境光，不等待后续处理；
...
每个爆点词必须是推文正文中连续、逐字存在且不超过 12 个字符的原文。共享层会丢弃改写、翻译、凭空添加、重复、长句或超过 5 个的结果；前端只包裹原有文本节点，不替换文案。
...
```text
dist/dumber-0.3.10.zip
dist/dumber-0.3.10.zip.sha256
dist/dumber-0.3.10-source.zip
dist/dumber-0.3.10-source.zip.sha256
```
