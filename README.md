...
- 首轮命中后立即出现以红、橙、洋红为主的虹彩边缘和环境光，不等待后续处理；
- 独立的第二轮模型请求只从命中正文中选出 2–5 个原文短语，以错峰放大、摆动和亮度脉冲强化情绪钩子；
- 正文获得稳定、高对比度的阅读平面；
- 保留 X 原有的字号、字重、行距、换行和卡片尺寸，不因标记触发布局重排；
- 时间、播放量和次要按钮被适度弱化，但功能不被删除；
- 全页面只维护一个 hover sidecar，自动停靠在卡片外侧；
- 右下角常驻 AI 活动指示器，实时显示扫描、排队、分析、完成或重试状态；
...
当前检测是纯文本的：图片与视频不会发送给模型，也没有 OCR、抽帧、音频、字幕或媒体 URL 提取。纯图片/视频帖子会跳过；图文或视频帖只按推文正文、引用正文和链接标题判断。
...
每个短语必须是推文正文中连续、逐字存在的原文。共享层会丢弃改写、翻译、凭空添加、重复、过长或超过 5 个的短语；前端只包裹原有文本节点，不替换文案。

UI 标签由本地 `primaryDriver` 映射生成，不再要求模型生成重复文案。共享层会过滤未知 ID与越界分数。

模型的 `promote` 字段仅为协议兼容提示；共享层会根据两项分数确定性生成宽候选，避免模型返回互相矛盾的布尔值。最终推广必须同时满足：刺激分达到当前门槛、持久价值低于当前上限，且用户没有对同一内容选择“这条真有用”。“挑选严格度”滑杆同时控制两道门：最左为刺激分 `>= 0.20`、持久价值 `<= 0.80`；默认仍为 `>= 0.72 / <= 0.45`；最右为 `>= 0.95 / <= 0.25`。最左档会纳入低唤醒但依靠权威、时效、猜测或反应链占用注意力的信息噪音；默认档继续保护项目、研究、作品和教程。
...
- 每条队列内部批次严格串行；首轮与二轮各最多一个活动批次，避免请求风暴；
- 首轮策展和二轮文案强化使用独立单飞队列，二轮失败不会撤销已经出现的精选边框，也不会阻塞后续首轮判定；
- 二轮只对最终命中内容发起，最多 8 条一批，并与首轮判定一起持久缓存；因此首次命中的内容通常会产生第二次模型调用；
...
src/curator.js                两阶段提示词、JSON Schema、响应解析、单飞批处理器
src/background.js             两阶段队列、API、超时重试、缓存、BYOK、偏好与指标
...
- 首轮边框不等待二轮、二轮精确原文校验与持久缓存；
- 情绪短语动画、reduced-motion、DOM 替换修复和取消精选后的文本节点还原；
...
```text
dist/dumber-0.3.9.zip
dist/dumber-0.3.9.zip.sha256
dist/dumber-0.3.9-source.zip
dist/dumber-0.3.9-source.zip.sha256
```
